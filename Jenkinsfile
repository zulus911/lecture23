...
//          echo "${tool 'maven'}"
    ...
    stage('Build'){
        withEnv(["PATH=${env.PATH}:${tool 'maven'}/bin"]){
            ...
            appName = sh(script: "mvn help:evaluate -Dexpression=project.name", returnStdout: true).trim()
            appVersion = sh(script: "mvn help:evaluate -Dexpression=project.version", returnStdout: true).trim()
        }        
    }
    stage('Test'){
        withEnv(["PATH=${env.PATH}:${tool 'maven'}/bin"]){
